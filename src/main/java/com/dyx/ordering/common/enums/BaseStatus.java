...
public enum BaseStatus {
    ...
    HDFS_OPERATION_ERROR(10006, "storage operation error", "存储文件操作错误"),
    ...
    MASTER_NOT_EXISTS(10025, "There is no normal master node", "没有正常的master节点"),
    TASK_DEFINITION_IS_INVALIDATED(10026, "TaskDefinition is invalidated: {0}", "任务定义校验失败: {0}"),
    TIMER_EXPIRES(10214, "The timing is not within the effective range：【{0}】", "定时【{0}】不在生效范围内"),
    ...
    QUERY_WORKFLOW_LEVEL_IN_LINEAGE_ERROR(1016101, "query workflow level in lineage error", "查询血缘中的工作流层级失败"),

    QUERY_WORKFLOW_DATA_LINEAGE_ERROR(1016102, "query workflow data lineage error", "查询血缘中的工作流数据失败"),

    QUERY_WORKFLOW_LINEAGE_ERROR_NOT_EXIST_PROCESS_INSTANCE(1016102,
            "query workflow lineage error, not exist process instance [{0}]", "查询血缘失败，不存在工作流实例[{0}]"),
    ...
    TENANT_CODE_CAN_NOT_BE_DEFAULT(10164, "Can not use the specify tenant code",
            "不能使用特殊租户名"),
    ...
    PROCESS_DEFINITION_EXCEEDING_THE_LIMIT(10168, "process definition exceeding the limit", "license工作流定义超出限额"),
    ...
    LISTING_EXECUTING_TASK_EXECUTION_CONTEXT_BY_WORKER_ERROR(10202,
            "listing executing TaskExecutionContext by worker address error", "查询Worker中正在执行的任务上下文信息错误"),
    QUERY_TASK_EXECUTION_CONTEXT_DETAIL_ERROR(10203, "query task execution context detail error",
            "查询Worker中的task上下文详情错误"),
    ...
    COMMAND_ALREADY_EXIST(10206,
            "The related workflow instance is already exist a operation, please try later",
            "对应的工作流实例已经有一个执行操作，请稍后再试"),
    QUERY_EXECUTING_WORKFLOWS_DETAIL_BY_ID(10207, "query executing workflows detail by id error",
            "查询Master中运行的工作流实例详情错误"),

    CLEAN_TASK_INSTANCE_ERROR_CURRENT_TASK_INSTANCE_IS_IN_ISOLATION_CHAIN(10208,
    ...
    CLEAN_TASK_INSTANCE_ERROR_CURRENT_TASK_INSTANCE_IS_IN_PAUSE(10209,
            ...
            "清除任务实例状态错误，当前的任务实例在加权或隔离暂停中，无需清除重跑，条件允许后将自动恢复"),

    PROJECT_CODE_NOT_EXIST(10210, "This project {0} was not found. Please refresh page.", "项目{0}不存在,请刷新页面"),
    CLEAN_TASK_INSTANCE_ERROR_UPSTREAM_WORKFLOW_INSTANCE_IS_RUNNING(10211,
    ...
    DEPENDENT_RUN_ERROR(10212,
            "dependent run error",
            "依赖链运行操作失败"),

    LOGIN_STYLE_ERROR(10213, "Get user login page style error", "获取用户登陆页面失败"),
    ...
    TASK_INSTANCE_NOT_DYNAMIC_TASK(10206, "task instance {0} is not dynamic", "任务实例[{0}]不是Dynamic类型"),

    CLEAN_TASK_INSTANCE_ERROR_TASK_STATUE(10206, "The current task status [{0}] does not allow clearing operations", "当前任务状态[{o}]不允许清除操作"),

    DELETE_BE_PROCESS_CITED_WORKER_GROUP_FAIL(10207,
            "delete worker group by id fail, for there are {0} process definition using it",
            "删除Worker分组失败，有[{0}]个工作流定义正在使用"),

    DELETE_BE_TASK_CITED_WORKER_GROUP_FAIL(10208,
            "delete worker group by id fail, for there are {0} task definition using it",
            "删除Worker分组失败，有[{0}]个任务定义正在使用"),
    ALERT_GROUP_NAME_UNLAWFUL(10209, "alert group name unlawful: 【{0}】", "告警组名称: 【{0}】，不是一个合法的名称"),

    UDF_FUNCTION_NOT_EXIST(20001, "UDF function not found", "UDF函数不存在"),
    UDF_FUNCTION_EXISTS(20002, "UDF function already exists", "UDF函数已存在"),
    RESOURCE_DATA_INTEGRITY_VIOLATION(20003, "resource data integrity violation, column value too long", "资源数据完整性冲突，部分字段值过长"),
    ...
    RESOURCE_IS_USED(20014, "resource file {0} is used by workflow {1}", "资源文件 [{0}] 被工作流 [{1}] 使用了"),
    ...
    PARENT_RESOURCE_PROJECT_OR_GLOBAL_DIFFERENT(20028, "parent resource belong project or global is different", "当前文件和父资源所属项目或全局资源属性不一致"),

    UPLOAD_FOLDER_EXIST(20029, "uploaded folder or inner folder already exists, are you sure to overwrite it", "上传的文件夹或内层文件夹已存在，是否确认覆盖"),
    UPLOAD_FILE_EXIST(20030, "uploaded file already exists, are you sure to overwrite it", "上传的文件已存在，是否确认覆盖"),

    USER_NO_OPERATION_PERM(30001, "user has no operation privilege", "当前用户没有操作权限"),
    USER_NO_OPERATION_PROJECT_PERM(30002, "User [{0}] does not have this operation permission under the [{1}] project",
            "用户[{0}]在[{1}]项目下无此操作权限"),

    USER_CAN_NOT_OPERATION(30003, "User [{0}] does not have permission for Project [{1}]", "用户[{0}]在项目[{1}]下无此权限"),
    ...
    SUB_PROCESS_DEFINE_RELEASE(60004, "process definition {0}'s sub process {1} version {2} on line", "工作流定义[{0}]所含的子工作流[{1}]版本[{2}]是上线状态"),
    ...
    DELETE_ALERT_GROUP_ERROR_HAS_PROCESS_DEFINITION_ASSOCIATED(110014,
            "the deletion of an alarm group failed, and there is a workflow associated with the alarm group",
            "删除告警组失败，并且存在与告警组关联的工作流"),
    ...
    TASK_GROUP_NAME_EXSIT(130001, "this task group name is repeated in a project", "该资源池名称在一个项目中已经使用"),
    TASK_GROUP_SIZE_ERROR(130002, "task group size error", "资源池大小应该为不小于0的整数"),
    TASK_GROUP_STATUS_ERROR(130003, "task group status error", "资源池已经被关闭"),
    TASK_GROUP_FULL(130004, "task group is full", "资源池已经满了"),
    TASK_GROUP_USED_SIZE_ERROR(130005, "the used size number of task group is dirty", "资源池使用的容量发生了变化"),
    TASK_GROUP_QUEUE_RELEASE_ERROR(130006, "failed to release task group queue", "资源池资源释放时出现了错误"),
    TASK_GROUP_QUEUE_AWAKE_ERROR(130007, "awake waiting task failed", "资源池使唤醒等待任务时发生了错误"),
    CREATE_TASK_GROUP_ERROR(130008, "create task group error", "创建资源池错误"),
    UPDATE_TASK_GROUP_ERROR(130009, "update task group list error", "更新资源池错误"),
    QUERY_TASK_GROUP_LIST_ERROR(130010, "query task group list error", "查询资源池列表错误"),
    CLOSE_TASK_GROUP_ERROR(130011, "close task group error", "关闭资源池错误"),
    START_TASK_GROUP_ERROR(130012, "start task group error", "启动资源池错误"),
    QUERY_TASK_GROUP_QUEUE_LIST_ERROR(130013, "query task group queue list error", "查询资源池队列列表错误"),
    TASK_GROUP_CACHE_START_FAILED(130014, "cache start failed", "资源池相关的缓存启动失败"),
    ...
    TASK_GROUP_QUEUE_ALREADY_START(130017, "task group queue already start", "节点已经获取资源池资源"),
    TASK_GROUP_STATUS_CLOSED(130018, "The task group has been closed.", "资源池已经被关闭"),
    TASK_GROUP_STATUS_OPENED(130019, "The task group has been opened.", "资源池已经被开启"),
    ...
    TASK_GROUP_USED_SIZE_NOT_EMPTY(130032, "the used size number of task group is not empty", "资源池已使用容量不为空"),
    ...
    TENANT_CODE_IS_NULL(1300016, "TENANT_CODE_IS_NULL", "租户编码不能为空"),
    ...
    BACK_FILL_PARALLEL_INVALIDATE(1400010, "PARALLEL {0} should > 0", "并行度 {0} 需要大于0"),

    ISOLATION_TASK_NOT_EXIST(1500000, "Isolation task not exist", "隔离任务不存在"),

    ISOLATION_TASK_USER_NO_PERMISSION(1500001, "Current login user does not have isolation view permission",
            "当前用户不具备当前项目隔离列表查看权限"),
    ...
    ISOLATION_TASK_SUBMIT_USER_NO_PERMISSION(1500106,
    ...
    ISOLATION_TASK_CANCEL_USER_NO_PERMISSION(1500304, "Current login user does not have isolation cancel permission",
            "当前用户不具备当前项目隔离取消权限"),
    ...
    CORONATION_TASK_PARSE_ERROR(1600000, "Coronation task parse error", "解析加权任务异常"),
    CORONATION_TASK_NOT_EXIST(1600001, "Coronation task not exist", "加权任务不存在"),
    CORONATION_TASK_PARSE_ERROR_TASK_NODE_NAME_IS_NOT_VALIDATED(1600002,
            "Coronation task parse error, taskNode name is not validated", "解析加权任务异常， 任务名不正确"),

    CORONATION_TASK_PARSE_ERROR_TASK_NODE_ALREADY_EXIST(1600003,
            "Coronation task parse error, workflowInstance: [{0}] taskNode:[{1}] is already in coronation",
            "解析加权任务异常， 工作流实例: [{0}] 任务: [{1}] 已经被加权"),

    CORONATION_TASK_USER_NO_PERMISSION(1600004, "Current login user does not have coronation view permission",
            "当前用户不具备当前项目加权列表查看权限"),

    CORONATION_TASK_SUBMIT_ERROR(1600100, "Coronation task submit error", "提交加权任务异常"),
    CORONATION_TASK_SUBMIT_USER_NO_PERMISSION(1600101, "Current login user does not have coronation submit permission",
            "当前用户不具备当前项目加权提交权限"),
    CORONATION_TASK_SUBMIT_TO_DB_ERROR(1600102, "Coronation task submit to db error: [{0}]", "提交加权任务到数据库异常: [{0}]"),
    CORONATION_TASK_ONLINE_ERROR(1600200, "Coronation task online error", "上线加权任务异常"),
    CORONATION_TASK_CANCEL_ERROR(1600300, "Coronation task cancel error", "取消加权任务异常"),
    CORONATION_TASK_CANCEL_USER_NO_PERMISSION(1500301, "Current login user does not have coronation cancel permission",
            "当前用户不具备当前项目加权取消权限"),
    CORONATION_TASK_LISTING_ERROR(1600400, "Coronation task listing error", "查询加权任务异常"),

    OPERATION_ALREADY_EXIST(1800000, "operation already exist", "操作已存在"),

    DATA_SOURCE_HAD_USED(1600000, "data source already used (projectName - workflowName - taskName):{0}",
            "数据源正在被使用(项目名 - 工作流名 - 任务名):{0}"),

    COMPLEMENT_CALENDAR_ERROR(1700000, "complement calendar error", "补数日历错误"),

    COMPLEMENT_MISSING_CALENDAR_PARAM(1700001, "complement missing calendar param", "补数缺少日历参数"),

    COMPLEMENT_QUERY_DATE_LIST_ERROR(1700002, "complement query date list error", "补数查询日期列表错误"),

    GLOBAL_PARAM_PROP_IS_EMPTY(1700003, "global param prop is empty", "全局参数prop为空"),

    BATCH_COPY_RESOURCE_NOT_EXIST(1700004,
            "workflow is copied to the resource under the target project without permission: type {0} name {1}",
            "工作流复制到目标项目下资源无权限：类型 {0} 名称 {1}"),

    PARENT_ONLINE_CHILD_OFFLINE(1700005,
            "parent process is online, but the child process [{0}] is offline",
            "父工作流上线，子工作流 {0} 未上线"),

    ARGUMENT_VALIDATE_FAILED(1700006,
            "argument validate failed, field: {0}, cause: {1}",
            "参数校验失败，字段：{0}， 原因：{1}"),

    JSON_FIELD_VALIDATE_FAILED(-1700007, "json field [{0}] format error", "json 字段 [{0}] 格式错误"),

    DATE_FORMAT_ERROR(-1700008, "date format error", "日期格式不正确"),

    PROJECT_RESOURCE_SHARE_EXISTED(1800001, "there are shared resources, please deal with them first.",
            "当前项目下存在共享资源，为了不影响其他项目的使用，请先前往处理"),

    START_END_TIME_REQUIRED(1800002, "start end time required, prevent slow query",
            "开始结束时间必填，以防慢查询"),

    TASK_SAVEPOINT_ERROR(1900001, "task savepoint error", "任务实例savepoint错误"),
    TASK_STOP_ERROR(1900002, "task stop error", "任务实例停止错误"),
    START_TASK_INSTANCE_ERROR(1900003, "start task instance error", "运行任务流实例错误"),
    LICENSE_IS_NULL_MODEL(2000002, "the license is null", "license不能为空"),
    WS_TASK_DEFINITION_EXCESS(2000003, "scheduling task definition excess", "license调度任务定义超出限额"),
    WT_TASK_DEFINITION_EXCESS(2000004, "sync task definition excess", "license同步任务定义超出限额"),

    DOWNSTREAM_WORKFLOW_HAS_NOT_CAN_COPY_WORKFLOW(2000005,
            "None of the tasks included in the downstream workflow support replication, downstream workflow name {0}",
            "下游工作流中包含的任务均不支持复制，下游工作流名称 {0}"),
            ;
